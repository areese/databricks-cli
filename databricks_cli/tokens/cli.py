...
def create_token_cli(api_client, lifetime_seconds, comment):
    ...
    This call returns QUOTA_EXCEEDED if the caller exceeds the token quota, which is 600.
    """
...
def revoke_cli(api_client, token_id):
    """
    Revoke an access toke
    'This call returns RESOURCE_DOES_NOT_EXIST if a token with the specified ID is not valid.
    """
